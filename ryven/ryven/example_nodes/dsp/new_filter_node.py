...
class MyApp(QWidget):
    ...
    def initUI(self, response_type, design_method, method_options, window_options, filter_order, fs, fc, fpass, fstop, wpass, wstop):
        ...
        self.response_type.addItems(["Lowpass", "Highpass", "Bandpass", "Bandstop"])
        ...
        self.fpass2 = QDoubleSpinBox()
        self.fpass2.setRange(1, 100000)
        self.fpass2.setDecimals(2)
        self.fpass2.setValue(1)  # You'll need to add fpass2 to the __init__ method's parameters

        self.fstop2 = QDoubleSpinBox()
        self.fstop2.setRange(1, 100000)
        self.fstop2.setDecimals(2)
        self.fstop2.setValue(1)  # You'll need to add fstop2 to the __init__ method's parameters

        self.fpass2.setDisabled(True)
        self.fstop2.setDisabled(True)
        ...
        left_layout.addWidget(QLabel("Fpass2"), 7, 0)
        left_layout.addWidget(self.fpass2, 7, 1)

        left_layout.addWidget(QLabel("Fstop2"), 8, 0)
        left_layout.addWidget(self.fstop2, 8, 1)
    ...
    def update_response_type(self):
        ...
        if response_type == "Highpass":
            ...
            self.fpass2.setDisabled(True)
            self.fstop2.setDisabled(True)
        elif response_type == "Bandpass" or response_type == "Bandstop":
            self.Fpass_label.setText("Fpass1")
            self.Fstop_label.setText("Fstop1")
            self.fc.setDisabled(True)
            self.fpass.setDisabled(False)
            self.fstop.setDisabled(False)
            self.fpass2.setDisabled(False)
            self.fstop2.setDisabled(False)
        ...
        if method == "IIR":
            if method_option == "Butterworth":
                if btype in ['low', 'high']:
                    ...
                    b, a = signal.butter(N, [self.fpass.value(), self.fpass2.value(), self.fstop.value(),
                                             self.fstop2.value()], btype=btype, fs=fs)
